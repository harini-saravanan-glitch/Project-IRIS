...
def main():
    print("--- Starting NLP Grooming Detection Model ---")
    ...
    TEST_CORPUS_XML = 'pan12-sexual-predator-identification-test-corpus-2012-05-21.xml'
    
    
    print("\n--- Step 0: Locating or Uploading Dataset Files ---")
    ...
    print("\n--- Step 4: Training and Evaluating Baseline LinearSVC model ---")
    ...
    print("\n--- Step 5: Training Advanced DistilBERT model ---")
    ...
    print("\n--- Step 6: Evaluating DistilBERT model performance ---")
    model.eval()
    ...
    def predict_grooming_intent(message, model_instance, tokenizer_instance):
        model_instance.eval()
        encoded_review = tokenizer_instance.encode_plus(
            message,
            max_length=128,
            add_special_tokens=True,
        ...
        if prediction == 1:
            label = "Predatory Intent Detected"
        else:
            label = "Normal"
        
        return {"prediction": label, "confidence": f"{confidence:.2%}"}

    print("\n--- Step 8: Testing prediction on the unlabeled test corpus ---")
    test_corpus_path = get_file(TEST_CORPUS_XML)
    print(f"Parsing unlabeled test conversations from: {test_corpus_path}")
    df_test_unlabeled = parse_conversations(test_corpus_path)
    
    if not df_test_unlabeled.empty:
        print(f"Successfully loaded {len(df_test_unlabeled)} unlabeled test messages.")
        sample_size = 10
        dynamic_test_messages = df_test_unlabeled['text'].sample(n=sample_size, random_state=42)
        
        print(f"\nDynamically testing {sample_size} random messages from the test set...\n")
        for msg in dynamic_test_messages:
            result = predict_grooming_intent(msg, model, tokenizer)
            print(f"Message: '{msg}'")
            print(f"  -> Prediction: {result['prediction']} (Confidence: {result['confidence']})\n")
    else:
        print("Could not load any messages from the test corpus.")
